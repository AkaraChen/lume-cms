...
Markdown and MDX use YAML frontmatter. JSON may be one object or an array of objects; its optional `body` field is Markdown. Both inputs use Fumadocs' public MDX plugins with the official starter baseline: GFM, heading IDs, images, npm install blocks, Shiki code highlighting, and table-of-contents extraction. The compiler stores a deterministic MDX function body in JSON; Markdown also exposes a compatibility HTML field. Raw HTML in Markdown is deliberately discarded; do not enable `allowDangerousHtml`/`rehype-raw` without adding an explicit sanitization policy. Run `lume-cms build` to generate stable JSON. Entries and object keys are sorted, paths are relative, line endings are stable, and no timestamp or machine path is emitted.

`remarkPlugins` and `rehypePlugins` in `lume.config.ts` accept either an array of extra unified plugins or a function that receives the complete default list and returns a composed list. Array additions are inserted at the same extension points as the official Fumadocs preset; use the function form only when you need to reorder or remove defaults.
