...
Request-time publishing requires `export const dynamic = 'force-dynamic'` for every consumer that reads a time-dependent collection: page details, layouts/navigation, lists, RSS, search, metadata/OG, text exports, and sitemap. Every dynamic server component and route handler must load its source inside the request; never retain an awaited loader instance at module scope. A route that reads only collections without time-dependent plugins can keep the official starter's `revalidate = false` and `generateStaticParams()` model. Single-source search can use `createFromSource(getSource)`. Multi-source search should use `createSearchAPI('advanced', { indexes })`, tag every collection's pages, and create the search API inside the request so a future post appears without a redeploy.

Sitemaps are metadata routes and static by default, so a sitemap that includes a scheduled collection must also export `dynamic = 'force-dynamic'`. Next.js route-segment `revalidate` values must be statically analyzable, so the runtime's changing next deadline cannot be assigned to that export. Next.js 16 can express a runtime-derived lifetime with `cacheLife({ revalidate })`, but only after enabling Cache Components and moving cached consumers to the `use cache` model; route-segment `dynamic` and `revalidate` options are disabled in that mode. The example therefore keeps scheduled and mixed consumers request-time dynamic instead of presenting a partial cache migration as deadline-safe ISR. The `examples/` directory is the official Create Fumadocs starter with only the source wiring changed, plus the minimal RSS and sitemap routes required by this package's acceptance criteria.
